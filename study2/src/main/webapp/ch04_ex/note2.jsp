...
function sendOk() {
	...
	const friendList = document.querySelector('.item-left-list');
	const receiverList = document.querySelector('.item-right-list');
	const friendIds = friendList.querySelectorAll('input[name=userId]');
	const receiverIds = receiverList.querySelectorAll('input[name=userId]');
	...
	// 좌측 친구목록중 선택된 요소는 선택 해제
	for(let chk of friendIds) {
		chk.checked = false;
	}
	
	// 우측 받을사람 checkbox 요소는 서버로 전송하기 위해서 반드시 항목들이 선택되어 있어야 한다.
	for(let chk of receiverIds) {
		chk.checked = true;
	}
