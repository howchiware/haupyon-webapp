...
function memberOk() {
	...
	// 패스워드는 5~10자 이내
	p = /^[a-z]{5,10}$/i;	
	if(! p.test(f.userPwd.value.trim())) {
		alert('패스워드를 입력하세요.')
		f.userPwd.focus();
		return;
	}
	
	if(! p.test(f.userPwd2.value.trim())) {
		alert('패스워드 확인을 입력하세요.')
		f.userPwd2.focus();
		return;
	}
	
	if(f.userPwd.value.trim() !== f.userPwd2.value.trim() ) {
		alert('패스워드가 틀렸습니다.')
		f.userPwd2.focus();
		return;
	}
	...
	if(! p.test(f.userName.value.trim())) {
		alert('⚠️이름을 입력하거나 올바른 형식으로 입력하세요.');
		f.userName.focus();
		return;
	}
	
	if(! isValidDate & f.birth.value.trim()) {
		alert('⚠️생일을 선택하세요');
		f.birth.focus();
		return;
	}
